...
int on_leave(unsigned player_ref, unsigned loc_ref) {
	sitter_t sitter;
	nd_get(sitter_hd, &sitter, &player_ref);
...
int on_will_attack(unsigned player_ref) {
	sitter_t sitter;
	nd_get(sitter_hd, &sitter, &player_ref);
...
int on_birth(unsigned ref, uint64_t v) {
	/* FIXME if I don't put here I gen a fatal error! */
	sitter_t sitter = STANDING;
	nd_put(sitter_hd, &ref, &sitter);
	return 0;
}
